...
-- Schema de negócio do Artemise (21 tabelas), portado de backend/migrations/001_schema.sql.
-- Diferenças vs Go: SEM tabela `sessions`; `usuarios` SEM `senha_hash` e SEM default no `id`
-- (o id passa a ser = auth.users.id, ligado em 0002). RLS e auth ficam em 0002; RPCs em 0003.
...
-- Perfil 1:1 com auth.users. `id` é fornecido (= auth.uid()), sem default. FK ligada em 0002.
CREATE TABLE usuarios (
  id            UUID PRIMARY KEY,
  clinica_id    UUID NOT NULL REFERENCES clinicas(id),
  nome          TEXT NOT NULL,
  email         TEXT NOT NULL,
  perfil_acesso TEXT NOT NULL DEFAULT 'admin' CHECK (perfil_acesso IN ('admin','recepção','profissional')),
